...
next
  ...
  next
    ...
    have "(c - 1) = Suc nat -1"
      ...
      ..
    also have "... = nat"
      apply(subst diff_Suc_1)
      ..
    finally have "c - 1 = nat"
      .

    have 3: "a * nat \<le> b * d"
      using 2
      apply(subst \<open>c - 1 = nat\<close>[symmetric])
      .
    ...
    also have "...  = a + a * nat"
      ...
      ..
    finally have 4: "a * c = a + a * nat"
      .

    have 5: "b * Suc d = b + b * d"
      apply (subst mult_Suc_right)
      ..

    have "a + a * nat \<le> b + b * d"
      using add_mono[OF \<open>a \<le> b\<close> 3]
      .
